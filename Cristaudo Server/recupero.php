...
if(isset($_POST['Mail'])){
	...
	if($errore==0){
		...
		$header= "From: Daily Life APP <info@cristaudo.altervista.org>\n";
		$header .= "Content-Type: text/html; charset=\"iso-8859-1\"\n";
		$header .= "Content-Transfer-Encoding: 7bit\n\n";
						
		$subject= "Daily Life - Nuovo PIN utente";
		
		$mess_invio="<html><body>";
		
		$mess_invio.="
		Clicca sul <a href=\"http://www.cristaudo.altervista.org/nuovo_pin.php?hash=".$hash."\">link</a> per confermare il nuovo PIN.<br />
		Se il link non è visibile, copia la riga qui sotto e incollala sul tuo browser: <br />
		http://www.cristaudo.altervista.org/nuovo_pin.php?hash=".$hash."
		";
		...
		if(@mail($_POST['Mail'], $subject, $mess_invio, $header)){
         	$sucRes->code=200;
         	$sucRes->msg="Mail inviata correttamente a: ".$_POST['Mail']." Segui le istruzioni riportate per ottenere un nuovo PIN id=".$userData[0]['IDUser'];
         	$sucRes->hash=$hash;
